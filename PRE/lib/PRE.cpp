...
static cl::opt<bool> EnablePRE("enable-pre2",
...
bool GVN::processNonLocalLoad(LoadInst *LI) {
  ...
  for (unsigned i = 0, e = Deps.size(); i != e; ++i) {
    ...
    //**EDIT**if (isa<AllocaInst>(DepInst) || isMalloc(DepInst) ||
        // Loading immediately after lifetime begin -> undef.
    //**EDIT**    isLifetimeStart(DepInst)) {
...
bool GVN::processInstruction(Instruction *I) {
  ...
  LLVMContext *c = &getGlobalContext();  //**EDIT**
  ...
  if (SwitchInst *SI = dyn_cast<SwitchInst>(I)) {
    ...
    for (unsigned i = 1, e = SI->getNumCases(); i != e; ++i) {
      ...
      if (isOnlyReachableViaThisEdge(Parent, Dst, DT))
        Changed |= propagateEquality(SwitchCond, SI->findCaseValue(ConstantInt::get(Type::getInt32Ty(*c), i)).getCaseValue(), Dst);  //**EDIT**
    }
    return Changed;
  }
